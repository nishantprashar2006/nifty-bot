...
class PnlGuard:
    def __init__(self, daily_loss_cap: float, daily_profit_lock: float) -> None:
        # daily_loss_cap is a NEGATIVE rupee figure
        self._loss_cap = float(daily_loss_cap)
        self._profit_lock = float(daily_profit_lock)
        self._realized = 0.0
    ...
    @property
    def profit_lock(self) -> float:
        return self._profit_lock
    ...
    def evaluate(self, unrealized: float = 0.0) -> GuardVerdict:
        with self._lock:
            ...
            if total >= self._profit_lock:
                return GuardVerdict(True, f"daily_profit_lock_hit({total:.2f}≥{self._profit_lock:.2f})")
